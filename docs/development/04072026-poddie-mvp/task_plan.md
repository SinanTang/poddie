...
- [x] Intel Mac support (2026-07-06, two small changes): (1) resolveTool probes the Intel ffmpeg-full keg `/usr/local/opt/ffmpeg-full/bin` alongside the arm one; (2) build is now UNIVERSAL (`--universal` in the dist scripts → one bundle with x86_64 + arm64 slices, output dist/mac-universal/). videotoolbox→libx264 fallback already covered Intel encoding. Verified: `lipo -info` shows both slices in the main binary AND Electron Framework; app copied out of the DMG (drag-to-Applications flow) launches with React mounted + CSS applied.
- [ ] Distribution blockers for other people's machines:
  ...
  — No Developer ID/notarization: recipients hit Gatekeeper "unidentified developer" (right-click→Open once). The ad-hoc universal signature also fails `codesign --verify --strict` on Electron's baked-in FinderInfo (harmless — AMFI still launches it), which a real Developer ID + notarization would fix. Fine for beta; notarize if it goes wide.
...
| Universal (Intel+ARM) build failed / signed badly — three-stage saga. (a) `makeUniversalApp` errored "Expected all non-binary files to have identical SHAs … CodeResources did not": our afterPack ad-hoc re-signed EACH per-arch `*-temp` build, so their signatures differed and the lipo merge refused. (b) After skipping `-temp` (sign only the merged bundle), `codesign --verify --strict` failed on `com.apple.FinderInfo` "detritus": `codesign --deep` re-adds FinderInfo to Electron's nested framework/helper bundles. (c) Stripping FinderInfo wouldn't stick — `xattr -cr` and batched `find -exec xattr -d {} +` both left some, because (i) `xattr -d` takes ONE file per call so `+` silently processed only the first, and (ii) Electron bakes FinderInfo into its framework bundles and DMG creation restores it | 3 | afterPack skips `-temp` dirs (`appOutDir.endsWith('-temp')`) so only the merged bundle is signed. FinderInfo can't be reliably cleaned for an ad-hoc universal build and is left as-is: it fails `--verify --strict` but does NOT block launch (AMFI accepts ad-hoc). Verified by copying the app out of the DMG (drag-to-Applications flow) and launching via CDP — React mounted, CSS applied. Proper fix = Developer ID + notarization (Phase 7 blocker). Lesson: don't hand-sign per-arch inputs to a universal merge; `xattr -d` is single-file; "launches" (AMFI) ≠ "passes --verify --strict" (Gatekeeper) |
| Packaged app rendered a white page with raw CSS text ("…e: 12px; }") after a UI change (user screenshot). Looked like a broken bundle, but the built out/ AND the on-disk asar both rendered fine when probed (harness on out/renderer; CDP `--remote-debugging-port` on the packaged binary → React mounted, `getComputedStyle(body).backgroundColor` = rgb(27,29,33), new labels present). Real cause: `npm run dist:dir` was run while `npm run dev` was still live — electron-vite dev and the dist build both write `out/`, so the packaged `index.html` was captured mid-write (its linked CSS renders as document text when the HTML is truncated). Same class as the export/hot-reload race already logged | 1 | Quit `npm run dev` before `npm run dist:dir`; clean rebuild from a quiet tree verified good via CDP. Diagnostic that cut through the theorizing: launch the packaged binary with `--remote-debugging-port=9222`, then read `document.body.innerText` / computed styles over CDP (Node 22 global WebSocket) — proves in seconds whether it's the artifact or a transient. Lesson: don't run a packaging build while the dev server is running |
